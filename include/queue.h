...
static inline bool batch_dequeue(queue_t *queue, uint32_t *item, uint32_t *n)
{
    ...
    *n = (uint32_t)(queue->cnt < *n ? queue->cnt : *n);
    ...
    uint64_t head = queue->head & (RQ_SIZE - 1);
    uint64_t available = RQ_SIZE - head;
    if (*n <= available) {
    ...
    queue->head = (queue->head + *n) & (RQ_SIZE - 1);
    queue->cnt -= *n;
    unlock_q(queue);
    return true;
}
...
{
    lock_q(queue);
    if ((RQ_SIZE - queue->cnt) < n) {
        unlock_q(queue);
        return false;
    }
    uint64_t tail = queue->tail & (RQ_SIZE - 1);
    uint64_t available = RQ_SIZE - tail;
    if (n <= available) {
    ...
    queue->tail = (queue->tail + n) & (RQ_SIZE - 1);
    queue->cnt += n;
    unlock_q(queue);
    return true;
}
...
static inline bool dequeue(queue_t *queue, uint32_t* item)
{
    ...
    *item = queue->tasks[queue->head];
    queue->head = (queue->head + 1) & (RQ_SIZE - 1);
    queue->cnt--;
    unlock_q(queue);
    return true;
}
...
{
    lock_q(queue);
    if (queue->cnt >= RQ_SIZE) {
        unlock_q(queue);
        return false;
    }
    queue->tasks[queue->tail] = item;
    queue->tail = (queue->tail + 1) & (RQ_SIZE - 1);
    queue->cnt++;
    unlock_q(queue);
    return true;
}
